...
class PDF extends FPDF
{
    ...
    function Header()
    {
        ...
        $this->Cell(190, 15, utf8_decode('DIRECTORIO DE CLIENTES'), 0, 1, 'C'); // Cambiado el título a mayúsculas y el alto de la celda a 15
        
        // Descripción debajo del título
        $this->SetFont('Times', '', 12); // Tamaño de fuente para la descripción
        $this->SetTextColor(0, 0, 0); // Color negro para la descripción
        $this->Cell(50); // Espacio para el logo
        $this->Cell(190, 10, utf8_decode('Este es un directorio detallado de los clientes registrados en el sistema.'), 0, 1, 'C');
    ...
    function FancyTable($header, $data)
    {
        ...
        $w = array(10, 60, 60); // Ajuste de anchos de columna
        ...
        foreach($data as $row)
        {
            ...
            $this->Cell($w[2],6,$row['telefono'],'LR',0,'C',$fill); // Centrando los campos
            $this->Ln();
            $fill = !$fill;
        }
        // Línea de cierre
...
// Consulta para obtener el directorio de clientes
$query = "SELECT nombre, telefono FROM cliente ORDER BY nombre ASC";
