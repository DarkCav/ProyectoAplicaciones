...
class PDF extends FPDF
{
    ...
    function Header()
    {
        ...
        $this->Cell(190, 15, utf8_decode('REPORTE DE PRODUCTOS CON MENOS STOCK'), 0, 1, 'C'); // Cambiado el título a mayúsculas y el alto de la celda a 15
        
        // Descripción debajo del título
        $this->SetFont('Times', '', 12); // Tamaño de fuente para la descripción
        $this->SetTextColor(0, 0, 0); // Color negro para la descripción
        $this->Cell(50); // Espacio para el logo
        $this->Cell(190, 10, utf8_decode('Este es un reporte detallado de los 10 productos con menos stock registrados en el sistema.'), 0, 1, 'C');
    ...
    function FancyTable($header, $data)
    {
        ...
        $w = array(10, 30, 50, 20, 20, 20, 40); // Ajuste de anchos de columna
        ...
        foreach($data as $row)
        {
            ...
            $this->Cell($w[2],6,utf8_decode($row['descripcion']),'LR',0,'C',$fill); // Centrando los campos
            $this->Cell($w[3],6,$row['precio'],'LR',0,'C',$fill); // Centrando los campos
            $this->Cell($w[4],6,$row['stock'],'LR',0,'C',$fill); // Centrando los campos
            $this->Cell($w[5],6,$row['peso'],'LR',0,'C',$fill); // Centrando los campos
            $this->Cell($w[6],6,utf8_decode($row['imagen_url']),'LR',0,'C',$fill); // Centrando los campos
            $this->Ln();
            $fill = !$fill;
        }
        // Línea de cierre
...
// Consulta para obtener los 10 productos con menos stock
$query = "SELECT id_producto, nombre, descripcion, precio, stock, peso, imagen_url FROM producto ORDER BY stock ASC LIMIT 10";
...
$header = array('#', 'Nombre', 'Descripcion', 'Precio', 'Stock', 'Peso', 'Imagen URL');
