...
class PDF extends FPDF
{
    ...
    function Header()
    {
        ...
        $this->Cell(190, 15, utf8_decode('REPORTE DE TIPOS DE CARNE'), 0, 1, 'C'); // Cambiado el título a mayúsculas y el alto de la celda a 15
        
        // Descripción debajo del título
        $this->SetFont('Times', '', 12); // Tamaño de fuente para la descripción
        $this->SetTextColor(0, 0, 0); // Color negro para la descripción
        $this->Cell(50); // Espacio para el logo
        $this->Cell(190, 10, utf8_decode('Este es un reporte detallado de los tipos de carne registrados en el sistema.'), 0, 1, 'C');
    ...
    function FancyTable($header, $data)
    {
        ...
        $w = array(10, 30, 50); // Ajuste de anchos de columna
        ...
        foreach($data as $row)
        {
            ...
            $this->Cell($w[1],6,utf8_decode($row['id_tipo_producto']),'LR',0,'C',$fill); // Centrando los campos
            $this->Cell($w[2],6,utf8_decode($row['nombre']),'LR',0,'C',$fill); // Centrando los campos
            $this->Ln();
            $fill = !$fill;
        }
        // Línea de cierre
...
// Consulta para obtener los tipos de carne
$query = "SELECT id_tipo_producto, nombre FROM tipo_producto ORDER BY id_tipo_producto ASC";
...
$header = array('#', 'ID Tipo Producto', 'Nombre');
